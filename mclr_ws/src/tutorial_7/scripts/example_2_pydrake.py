...
parser = argparse.ArgumentParser(description='Run the pendulum swingup example.')
parser.add_argument('--torque_limit', type=float, default=sys.float_info.max, 
                    help='The torque limit for the pendulum swingup example.')
...
def pendulum_continous_dynamics(x, u):
    ...
    q_ddot = (-b * x[1] + u - m * g * l * np.sin(x[0])) / (m * l**2)
    x_dot = np.array([x[1], q_ddot])
    return x_dot
...
def pendulum_discretized_dynamics(x, u, x_next, dt):
    ...
    # residual to x_next
    x_dot = pendulum_continous_dynamics(x=x, u=u)
    x_next_euler = x + x_dot * dt
    residuals= x_next - x_next_euler
    return residuals
...
for k in range(N):
    prog.AddConstraint(control[k] <= torque_limit)
    prog.AddConstraint(control[k] >= -torque_limit)
