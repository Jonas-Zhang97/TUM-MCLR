...
def generate_foot_steps(foot_step_0, step_size_x, no_steps):
    ...
    for i in range(no_steps):
        pass
        foot_steps.append([foot_steps[-1][0] + step_size_x, foot_steps[-1][1] * (-1)**(i + 1)])
    return foot_steps
...
def plot_foot_steps(foot_steps, XY_foot_print, ax):
    ...
    for i, foot_step in enumerate(foot_steps):
        ...
        ax.add_patch(patches.Rectangle((foot_step[0], foot_step[1]), XY_foot_print[0], XY_foot_print[1], color=color))
    # set the axis limits
