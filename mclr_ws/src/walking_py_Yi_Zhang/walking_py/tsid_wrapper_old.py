...
class TSIDWrapper:
    ...
    def __init__(self, conf):
        ...
        feet_contact_vertices = np.zeros((3, 4))
        ...
        posture_ref = create_sample(conf.q_tsid)
        ...
        self.LH = robot.model().getFrameId(self.conf.lh_frame_name)
        ...
        self.leftHandTask.setKp(self.conf.kp_hand * np.ones(6))
        self.leftHandTask.setKd(2.0 * np.sqrt(self.conf.kp_hand) * np.ones(6))
        ...
        self.RH = robot.model().getFrameId(self.conf.rh_frame_name)
        ...
        self.rightHandTask.setKp(self.conf.kp_hand * np.ones(6))
        self.rightHandTask.setKd(2.0 * np.sqrt(self.conf.kp_hand) * np.ones(6))
        ...
        self.acc_sol = np.zeros(self.robot.nv)
    ...
    def comState(self):
        ...
        if self.acc_sol is not None:
            acc = self.comTask.getDesiredAcceleration
        else:
            acc = np.zeros(3)
    ...
    def set_RF_ref(self, pose, vel=None, acc=None):
        update_sample(self.rf_ref, pose, vel, acc)
        self.rightFootTask.setReference(self.rf_ref)

    def set_LF_ref(self, pose, vel=None, acc=None):
        update_sample(self.lf_ref, pose, vel, acc)
        self.leftFootTask.setReference(self.lf_ref)

    def set_RH_ref(self, pose, vel=None, acc=None):
        update_sample(self.rh_ref, pose, vel, acc)
        self.rightHandTask.setReference(self.rh_ref)

    def set_LH_ref(self, pose, vel=np.zeros(6), acc=np.zeros(6)):
        update_sample(self.lh_ref, pose, vel, acc)
        self.leftHandTask.setReference(self.lh_ref)

    ############################################################################
    # get endeffector references
    ############################################################################

    def get_RF_ref(self):
        return vectorToSE3(self.rf_ref.value()), self.rf_ref.derivative(), self.rf_ref.second_derivative()

    def get_LF_ref(self):
        return vectorToSE3(self.lf_ref.value()), self.lf_ref.derivative(), self.lf_ref.second_derivative()

    def get_RH_ref(self):
        return vectorToSE3(self.rh_ref.value()), self.rh_ref.derivative(), self.rh_ref.second_derivative()

    def get_LH_ref(self):
        return vectorToSE3(self.lh_ref.value()), self.lh_ref.derivative(), self.lh_ref.second_derivative()
    ...
    def get_LF_vel_acc(self):
        data = self.formulation.data()
        T_w = self.robot.framePosition(data, self.LF)
        v_w = self.robot.frameVelocity(data, self.LF)
        if self.dv_sol is not None:
            a_w = self.leftFootTask.getAcceleration(self.dv_sol)
            return T_w, v_w, a_w
        return T_w, v_w

    def get_RF_vel_acc(self):
        data = self.formulation.data()
        T_w = self.robot.framePosition(data, self.RF)
        v_w = self.robot.frameVelocity(data, self.RF)
        if self.dv_sol is not None:
            a_w = self.rightFootTask.getAcceleration(self.dv_sol)
            return T_w, v_w, a_w
        return T_w, v_w

    def get_LH_vel_acc(self):
        data = self.formulation.data()
        T_w = self.robot.framePosition(data, self.LH)
        v_w = self.robot.frameVelocity(data, self.LH)
        if self.dv_sol is not None:
            a_w = self.leftHandTask.getAcceleration(self.dv_sol)
            return T_w, v_w, a_w
        return T_w, v_w

    def get_RH_vel_acc(self):
        data = self.formulation.data()
        T_w = self.robot.framePosition(data, self.RH)
        v_w = self.robot.frameVelocity(data, self.RH)
        if self.dv_sol is not None:
            a_w = self.rightHandTask.getAcceleration(self.dv_sol)
            return T_w, v_w, a_w
        return T_w, v_w
    ...
    def get_RF_wrench(self):
        if self.formulation.checkContact(self.contactRF.name, self.sol):
            return self.formulation.getContactForce(self.contactRF.name, self.sol)
        else:
            return np.zeros(6)

    def get_RF_normal_force(self):
        return self.contactRF.getNormalForce(self.get_RF_wrench(self.sol))

    def get_LF_wrench(self):
        if self.formulation.checkContact(self.contactLF.name, self.sol):
            return self.formulation.getContactForce(self.contactLF.name, self.sol)
        else:
            return np.zeros(6)

    def get_LF_normal_force(self):
        return self.contactLF.getNormalForce(self.get_LF_wrench(self.sol))

    def get_RH_normal_force(self):
        if self.formulation.checkContact(self.contactRH.name, self.sol):
            f = self.formulation.getContactForce(self.contactRH.name, self.sol)
            return self.contactRH.getNormalForce(f)
        else:
            return 0.0

    def get_LH_normal_force(self):
        if self.formulation.checkContact(self.contactLH.name, self.sol):
            f = self.formulation.getContactForce(self.contactLH.name, self.sol)
