...
class TSIDWrapper:
    ...
    def __init__(self, conf):
        ...
        actuated_link_names = [
            link.name
            for link in list(self.model.frames)
            if link.type is pin.FrameType.JOINT
        ][1:]
        actuated_link_ids = [
            self.model.getFrameId(name) for name in actuated_link_names
        ]
        ...
        feet_contact_vertices[0, :] = [-conf.lfxn, -conf.lfxn, conf.lfxp, conf.lfxp]
        feet_contact_vertices[1, :] = [-conf.lfyn, conf.lfyp, -conf.lfyn, conf.lfyp]
        ...
        comTask.setKp(conf.kp_com * np.array([1, 1, 1]))
        comTask.setKd(2.0 * np.sqrt(conf.kp_com) * np.array([1, 1, 1]))
        ...
        amTask.setKp(conf.kp_am * np.array([1.0, 1.0, 10.0]))
        amTask.setKd(2.0 * np.sqrt(conf.kp_am * np.array([1.0, 1.0, 10.0])))
        ...
        self.leftFootTask.setKp(self.conf.kp_foot * np.array([1, 1, 1, 1, 1, 10]))
        self.leftFootTask.setKd(
            2.0 * np.sqrt(self.conf.kp_foot) * np.array([1, 1, 1, 1, 1, 10])
        )
        ...
        self.rightFootTask.setKp(self.conf.kp_foot * np.array([1, 1, 1, 1, 1, 10]))
        ...
        self.leftHandTask = tsid.TaskSE3Equality(
            "traj-left-hand", self.robot, self.conf.lh_frame_name
        )
        self.leftHandTask.setKp(100 * np.array([1, 1, 1, 0, 0, 0]))
        self.leftHandTask.setKd(2.0 * np.sqrt(100) * np.array([1, 1, 1, 0, 0, 0]))
        # formulation.addMotionTask(self.leftHandTask, self.conf.w_hand, 1, 0.0)
        self.lh_ref = create_sample(self.robot.framePosition(data, self.LH))
        ...
        self.rightHandTask = tsid.TaskSE3Equality(
            "traj-right-hand", self.robot, self.conf.rh_frame_name
        )
        self.rightHandTask.setKp(100 * np.array([1, 1, 1, 0, 0, 0]))
        self.rightHandTask.setKd(2.0 * np.sqrt(100) * np.array([1, 1, 1, 0, 0, 0]))
        # formulation.addMotionTask(self.rightHandTask, self.conf.w_hand, 1, 0.0)
        self.rh_ref = create_sample(self.robot.framePosition(data, self.RH))
        ...
        self.torsoTask.setKd(
            2.0 * np.sqrt(self.conf.kp_torso) * np.array([0, 0, 0, 1, 1, 1])
        )
        ...
        if conf.w_joint_bounds > 0.0:
            formulation.addMotionTask(jointVelBoundsTask, conf.w_joint_bounds, 0, 0.0)
    ...
    def get_LF_pose(self, dv=None):
        data = self.formulation.data()
        T_h_w = self.robot.framePosition(data, self.LH)
        return T_h_w

    def get_RF_pose(self, dv=None):
        data = self.formulation.data()
        T_h_w = self.robot.framePosition(data, self.RH)
        return T_h_w
    ...
    def add_contact_RF(self, transition_time=0.0):
        ...
        if not self.contact_RF_active:
            ...
            if self.conf.w_feet_contact >= 0.0:
                ...
                self.formulation.addRigidContact(self.contactRF, self.conf.w_force_reg)
            self.contact_RF_active = True
    ...
    def add_contact_LF(self, transition_time=0.0):
        ...
        if not self.contact_LF_active:
            ...
            if self.conf.w_feet_contact >= 0.0:
                ...
                self.formulation.addRigidContact(self.contactLF, self.conf.w_force_reg)
            self.contact_LF_active = True
