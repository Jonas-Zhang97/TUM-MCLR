...
q_actuated_home[14:21] = np.array([0, 0, 0.3, 0, 0, 0, 0])
# q_home = np.hstack([np.array([0, 0, 0.9, 0, 0, 0, 1]), q_actuated_home])

q_straight = np.zeros(na)
q_home = np.hstack([np.array([0, 0, 0.9, 0, 0, 0, 1]), q_straight])


q_tsid = np.zeros(na)
q_tsid[0:6] = np.array([0, 0, -0.44, 0.9, -0.45, 0])
q_tsid[6:12] = np.array([0, 0, -0.44, 0.9, -0.45, 0])
q_tsid[14:21] = np.array([0, -0.24, 0, -1, 0, 0, 0])
q_tsid[21:28] = np.array([0, -0.24, 0, -1, 0, 0, 0])
...
kp_hand = 100.0          # proportional gain of hand constraint
...
kp_posture = np.array([
        10., 10., 10., 10., 100., 100.,           # left leg  #low gain on axis along y and knee
        10., 10., 10., 10., 100., 100.,           # right leg #low gain on axis along y and knee
...
# step dimensions
step_size_x = 0.25 # 0.25              # step size in x direction
