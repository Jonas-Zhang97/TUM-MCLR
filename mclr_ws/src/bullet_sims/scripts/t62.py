...
class BalanceController():
    ...
    def ankleBalencingStategy(self, x, x_ref, K_x, K_p, p, dt, p_ref=np.zeros(3), x_ref_dot=np.zeros(3)):
        ...
        self.tw.comTask.setReference(ankle_traj)
    ...
    while not rospy.is_shutdown():
        ...
        if 13.5 > t > 4.:
            ...
            if strategy == 'ankle':
                ...
                bc.hipBalencingStategy(grp_data[1], 0.5)   # set to 7 can work with 90N force
            elif strategy == 'combined':
                if not info_prompted:
                    info_prompted = True
                    rospy.logwarn('========= COMBINED BALANCING STRATEGY ACTIVATED =========')
                bc.ankleBalencingStategy(com_state.pos(), x_ref, 1.5, 1, grp_data[0], dt, grp_ref_data[0])
                bc.hipBalencingStategy(grp_data[1], 0.5)
            elif strategy == 'none':
                if not info_prompted:
                    info_prompted = True    
        ...
        '''
        _,dv_sol = tw.update(q_tsid, v_tsid,t)

        q_tsid, v_tsid = tw.integrate_dv(q_tsid, v_tsid, dv_sol, dt)
...
if __name__ == '__main__': 
    ...
    strategy = rospy.get_param('~balance_strategy', 'none')
